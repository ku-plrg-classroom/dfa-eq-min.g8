...
case class DFA(
  ...
  def normalized: DFA = normalized(1)

  /** The normalized DFA with states from init to init + n */
  def normalized(init: State): DFA =
    if (states.max == init + states.size - 1 && states.min == init) this
    else
      val stateList = states.toList.sorted
      val n = stateList.length
      val newStates = (0 until states.size).toList.map(_ + init)
      val map = (stateList zip newStates).toMap
      val newTrans = (for {
        q <- stateList
        a <- symbols
